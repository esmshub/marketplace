...
import { findClubs, summarizeResults, updateClub, getPosition, updatePlayer, insertPlayer, insertClub } from "./helpers";
        ...
        for (const clubData of result.data) {
          ...
          const upsertPlayers = async (players: PlayerDto[], pendingClub: Promise<ClubGetPayload<null>>) => {
            ...
            for (const player of players) {
              ...
              const existingPlayer = existing?.players.find(p => p.name === player.name && p.nat === player.nat);
